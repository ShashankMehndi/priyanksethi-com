...
export function CaseStudyGrid({
  ...
  /** "carousel" = swipe (default); "grid" = legacy 3-up grid */
      ...
      <Container size="lg">
        ...
        {variant === "grid" ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
            {cases.map((c) => (
              <CaseCard key={c.slug} c={c} className="" />
            ))}
          </div>
        ) : (
          <Carousel ariaLabel={title} autoScroll>
            {cases.map((c) => (
              <CaseCard
                key={c.slug}
                c={c}
                className="shrink-0 basis-[78%] sm:basis-[44%] lg:basis-[30%]"
              />
            ))}
          </Carousel>
        )}
...
function CaseCard({ c, className }: { c: CaseEntry; className: string }) {
  return (
    <Link
      href={c.href ?? `/case-studies/case/${c.slug}`}
      className={`group relative block overflow-hidden rounded-2xl bg-section-warm ring-1 ring-section-alt transition hover:ring-brand-blue ${className}`}
    >
